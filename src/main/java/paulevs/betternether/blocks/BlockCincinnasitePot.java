...
public class BlockCincinnasitePot extends BlockCincinnasite
{
	...
	public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ)
	{
		...
		facing == EnumFacing.UP && world.getBlockState(pos.up()).getBlock() == Blocks.AIR)
		{
			...
			if (BlocksRegistry.BLOCK_POTTED_PLANT != Blocks.AIR && BlockPottedPlant.hasPlant(b))
			{
				IBlockState state2 = ((BlockPottedPlant) BlocksRegistry.BLOCK_POTTED_PLANT).getBlockState(b);
